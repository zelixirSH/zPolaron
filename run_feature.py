...
def score(graph, model_path="best_model.pth", output_file='features_128.pt'):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    
    # Ensure output file ends with .pt
    if not output_file.endswith('.pt'):
        output_file = output_file.rsplit('.', 1)[0] + '.pt'
    ...
    # Get model predictions
        ...
        features_128 = out['features_128']
        
        # Save 128-dimensional features to file
        torch.save(features_128, output_file)
        print(f"Saved 128-dimensional features to {output_file}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", type=str, 
                        ...
                        help="Model file.")
    parser.add_argument("-o", "--output", type=str, default="features_128.pt",
                        help="Output .pt file path for 128-dimensional features (default: features_128.pt).")
