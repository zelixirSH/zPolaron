...
def get_feature(ligand_mol2_file, receptor_pdb_file, mutations=None, cutoff=40., use_interaction=False):
    ...
    for chain_id, seq in protein.all_residue_1_letter.items():
        ...
        esm_embedding_dic[chain_id] = get_esm_embedding_with_gap_split(
            esm_model, batch_converter, mutated_sequence,
...
def score(graph, model_path="best_model.pth", output_file='output.txt', mutations=None):
    ...
    # Get model predictions
        ...
        # Process mutations (if specified)
        if mutations:
            mutation_set = {res.strip() for res in mutations.split(',')}
            for redesigned_res in mutation_set:
                chain = redesigned_res[0]
        ...
        # Extract predicted probabilities and labels at mutation positions
            ...
            # Compute mutation-position metrics
        ...
        # Add per-chain sequence info (if available)
        if reserved_chain_ids and chain_full_seq:
            output_content.append("\nPer-chain Sequence Details\n")
            output_content.append("="*50 + "\n\n")
            
            # Separate original sequences per chain
            chain_original_seqs = {}
            chain_mutated_seqs = {}
            ...
            for chain_id in chain_original_seqs.keys():
                ...
                # Find mutation positions in current chain
                    chain_mut_indices = []
                    ...
                    if chain_mut_indices:
                        ...
                        # Get residue position
                            ...
                            # Get original and mutated amino acids
                        ...
                        # Write output content to file and print to console
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model", type=str, 
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts/model_seq_score.pth"),
                        help="Model file.")
